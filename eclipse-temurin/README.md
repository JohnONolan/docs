...
-	[`17.0.12_7-jre-ubi9-minimal`, `17-jre-ubi9-minimal`](https://github.com/adoptium/containers/blob/f7ce125dc28d8c0d065d94829e63094daa018ca3/17/jre/ubi/ubi9-minimal/Dockerfile)
...
```console
$ docker run -v $(pwd)/certs:/certificates/ -e USE_SYSTEM_CA_CERTS=1 ppc64le/eclipse-temurin:21
```
...
```dockerfile
FROM ppc64le/eclipse-temurin:21
RUN mkdir /opt/app
...
COPY --from=ppc64le/eclipse-temurin:21 $JAVA_HOME $JAVA_HOME
...
FROM ppc64le/eclipse-temurin:21 as jre-build
...
```dockerfile
FROM ppc64le/eclipse-temurin:21.0.2_13-jdk
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
